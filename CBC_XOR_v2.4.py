...
# Ajouts :
#    - Ajout de logs détaillés pour chaque étape clé :
#        - Génération de clés et IV.
#        - Découpage des fichiers en blocs.
#        - Chiffrement et déchiffrement (avec progression visible).
#        - Sauvegarde des fichiers chiffrés ou déchiffrés.
#    - Introduction d’une boîte de dialogue personnalisée pour gérer les entrées utilisateur via Toplevel.
#    - Consolidation des logs pour inclure des messages de succès ou d’erreurs clairs.
#    - Ajustement des logs pour gérer des processus longs (comme le chiffrement de blocs multiples).
# Modifications :
#    - Refactorisation complète pour intégrer les logs sans perturber le flux logique.
#    - Remplacement de simpledialog par une solution personnalisée pour une meilleure intégration.